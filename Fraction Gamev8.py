...
def generateQuestion():
    ...
    question_label = Label(window, text=f"{question.num1}/{question.den1} {question.operator} {question.num2}/{question.den2}", font=("Helvetica", 48))
    question_label.place(x=495, y=300)
    ...
    def playGame():
        ...
        def submit(submit_num, submit_den):
            ...
            if not (submit_num.isdigit() and submit_den.isdigit()):
                print("Enter integers dummy, and no decimals or negatives")
            else:
                submit_num = int(submit_num)
                submit_den = int(submit_den)
                if (submit_num, submit_den) == answer:
                    print("Correct!")
                    number_correct += 1
                    ...
                    if number_correct < 5:
                        ...
                        endGame()
                elif (submit_num, submit_den) == (0, 0):
                    print("You're doing this on fucking purpose, aren't you. Don't do 0 over 0") #Otherwise this would produce an error
                elif simplifyFraction(submit_num, submit_den) == answer:
