...
def openStart():
    ...
    def playGame():
        ...
        def submit(num, den):
            ...
            '''def PlayerStep(dest_x):
                global player_x
                if player_x < dest_x:
                    player_x += 5
                    player.place(x=player_x, y=200)
                    window.after(12, lambda:PlayerStep(dest_x))
            '''
            def PlayerStep(steps): # With initial steps of 20, moves for a total of 200 units
                if steps != 0:
                    game_canvas.move(player, 5, 0)
                    window.after(2, lambda:PlayerStep(steps - 1))
        ...
        #PhotoImage(master = game_canvas, width = 1200, height = 800) #Doesn't work?
        game_canvas.place(x=0, y=0)
        '''
        Error: pyimage1/pyimage2 when attempting to hide canvas image
        How to replicate error:
        - Open instructions window and play a game OR
        - Open empty highscores window and play a game
        Interesting that highscores window must be empty, it is likely the placement of score labels that prevents the error somehow
        Problem solved now?: Switched from
        finish_line_image.image = finish_line_image
        to
        game_canvas.image = finish_line_image
        (for garbage collection prevention)
        '''
        ...
        '''
        #Code for standard label
        finish_line_label = Label(window, image=finish_line_image)
        finish_line_label.image = finish_line_image # Prevent garbage collection
        finish_line_label.place(x=1000, y=100)'''
        ...
        player = game_canvas.create_image(100,100,image=player_image)
        #global player_x, dest_x
        #player_x = 300
        #dest_x = 500
    ...
    AI_difficulty.place(x=560, y=400)
    # Play button
...
def openInstructions():
    ...
    instructions_window.config(bg="#400050") # Purple
    ...
    line8 = Label(instructions_window, text="At the end of the game, your score is recorded to the Highscores window",
                  font=("Helvetica", 12))
    #Notes and tips
    line9 = Label(instructions_window, text="If the answer is 0, enter 0 / 1",
                  font=("Helvetica", 12))
    line10 = Label(instructions_window, text="You can press tab and shift+tab to alternate between numerator and denominator",
                  font=("Helvetica", 12))
    line11 = Label(instructions_window, text="You can press enter to submit rather than pressing the submit button",
                  font=("Helvetica", 12)) #Not true yet, but will implement
...
def openHighscores():
    ...
    class score_label():
        def __init__(self, username, time_taken, year_level, AI_difficulty, win_status, y_cord):
            ...
            # Placing labels
            self.username_label.place(x=10, y=y_cord)
            self.time_taken_label.place(x=371, y=y_cord)
            self.year_level_label.place(x=504, y=y_cord)
            self.AI_difficulty_label.place(x=695, y=y_cord)
            self.win_status_label.place(x=886, y=y_cord)
    ...
    def load_scores():
        try:
            ...
            for data in data_list:
                score_label(data["username"], f"{data['time_taken']:.2f}s", data["year_level"], data["AI_difficulty"], data["win_status"], y_cord)
                y_cord += 50
                if y_cord > 500:
                    break
        except FileNotFoundError:
    ...
    time_label = Label(highscores_window, text="Time", font=("Helvetica", 24), width = 6)
    ...
    time_label.place(x=371, y=10)
    year_level_label.place(x=504, y=10)
    AI_difficulty_label.place(x=695, y=10)
    win_status_label.place(x=886, y=10)
    # Load scores
    load_scores()
    # Reset button
    reset_button = Button(highscores_window, text = "Reset", command = reset_scores, font = ("Helvetica", 24), padx = 10, pady = 10)
    reset_button.place(x=450, y=515)
...
def startUp():
    ...
    # Title
    title = Label(window, text = "Fractions", font = ("Helvetica", 72), fg="#FFFFFF", bg="#300040", padx = 10, pady = 10) #300040
    title.place(x=388, y=140)
    # Start button
