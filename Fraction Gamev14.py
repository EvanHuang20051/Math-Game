...
def openStart():
    ...
    def checkEntries():
        ...
        if username.get() == "":
            ...
            shorten_username=messagebox.showinfo("Username must be 18 characters or less!", "Username must be 18 characters or less!")
...
def openHighscores():
    ...
    class score_label():
        # Change this to lines, allow more space for username, use a monospaced font like courier?
        def __init__(self, username, time_taken, year_level, AI_difficulty, win_status, y_cord):
            self.username_label = Label(highscores_window, text=username, font=("Courier", 24), width = 17)
            self.time_taken_label = Label(highscores_window, text=time_taken, font=("Helvetica", 24), width = 6)
            self.year_level_label = Label(highscores_window, text=year_level, font=("Helvetica", 24), width = 8)
            ...
            # Placing labels
            self.username_label.place(x=13, y=y_cord)
            self.time_taken_label.place(x=360, y=y_cord)
            self.year_level_label.place(x=497, y=y_cord)
            self.AI_difficulty_label.place(x=690, y=y_cord)
            self.win_status_label.place(x=883, y=y_cord)
    ...
    username_label = Label(highscores_window, text="Username", font=("Helvetica", 24), width = 17)
    ...
    # Placing labels
    username_label.place(x=13, y=10)
    time_label.place(x=360, y=10)
    year_level_label.place(x=497, y=10)
    AI_difficulty_label.place(x=690, y=10)
    win_status_label.place(x=883, y=10)
    # Load scores
    load_scores()
    # Reset button
