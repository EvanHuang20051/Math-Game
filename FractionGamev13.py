...
def openStart():
    ...
    def checkEntries():
        ...
        if username.get() == "":
            ...
            #Start the game
            global base_list
            #global selected_year_level, selected_AI_difficulty
            #selected_year_level = year_level.get()
            #selected_AI_difficulty = AI_difficulty.get()
    ...
    def playGame():
        ...
        def record_score(username, time_taken, year_level, AI_difficulty, win):
            data = {"username": username, "time_taken": time_taken, "year_level": year_level, "AI_difficulty": AI_difficulty, "win_status": "Win" if win else "Loss"}
            try:
                ...
                data_list = []
            data_list.append(data) #Need to add function to insert to correct location
            #Rank by: 1. Win status, 2. Year level, 3. AI difficulty, 4. Time, 5. Recency. Use json to achieve this
            with open("scores.json", 'w') as file:
                json.dump(data_list, file) #json.dump(data_list, file, indent=4)
...
def openHighscores():
    ...
    class score_label():
        def __init__(self, username, time_taken, year_level, AI_difficulty, win_status, y_cord):
            self.username_label = Label(highscores_window, text=username, font=("Helvetica", 24), width = 9)
            self.time_taken_label = Label(highscores_window, text=time_taken, font=("Helvetica", 24), width = 9)
            ...
            self.win_status_label = Label(highscores_window, text=win_status, font=("Helvetica", 24), width = 9)
            #Placing labels
            self.username_label.place(x=10, y=y_cord)
            self.time_taken_label.place(x=210, y=y_cord)
            self.year_level_label.place(x=410, y=y_cord)
            self.AI_difficulty_label.place(x=610, y=y_cord)
            self.win_status_label.place(x=810, y=y_cord)

    def label_score(username, time_taken, year_level, AI_difficulty, win_status):
        score
        print(username, time_taken, year_level, AI_difficulty, win_status)
    ...
    highscores_window = Toplevel(window)
    ...
    load_scores()
    #Reset button
    reset_button = Button(highscores_window, text = "Reset", command = reset_scores, font = ("Helvetica", 30), padx = 10, pady = 10)
