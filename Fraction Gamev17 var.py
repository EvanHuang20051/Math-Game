...
def openInstructions():
    ...
    line10 = Label(instructions_window, text="You can press enter to submit rather than pressing the submit button",
                  font=("Helvetica", 12), width = 65, anchor = 'w') #Not true yet, but will implement soon
    ...
    def load_scores():
        try:
            ...
            for data in data_list:
                #Shortened words and mm:ss time format
                minute_seconds = time.strftime('%M:%S', time.gmtime(data['time_taken']))
                simple_year_level = data["year_level"][4:]
                simple_AI_difficulty = data["AI_difficulty"][0]
                simple_win_status = data["win_status"][0]
                score_label(data["username"], minute_seconds, simple_year_level, simple_AI_difficulty, simple_win_status, y_cord)
                y_cord += 50
                if y_cord > 500:
                    break
        except FileNotFoundError:
